 ...
 */
public class
        ProbeWorker implements Runnable {
    ...
    @Override
    public void run() {
        logger.debug(String.format("Measuring probe %s at %s", this.probe.getMeasurementName(), System.currentTimeMillis()));
        try {
            this.metricReporting.report(this.metricFactoryInterface.from(this.probe.getMeasurementName(), this.probe.getMeasurementValue()));
        } catch (MeasurementNotAvailableException e) {
            logger.error(String.format("Could not retrieve metric %s", probe.getMeasurementName()));
