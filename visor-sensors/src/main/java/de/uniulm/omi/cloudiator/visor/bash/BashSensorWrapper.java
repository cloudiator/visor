 ...
 * TODO: this class calls Process very often. It assumes that the
...
public class BashSensorWrapper extends AbstractSensor {
	...
	private String getSourceValue(SensorConfiguration sensorConfiguration) throws SensorInitializationException {
		Optional<String> type = sensorConfiguration.getValue(FILE_RETRIEVAL_SOURCE_KEY);
		if(type.isPresent()) {
			return type.get();
		}
		throw new SensorInitializationException("file retrieval source (" + FILE_RETRIEVAL_SOURCE_KEY + ") not set.");
	}
	
	private File provideFile(SensorConfiguration sensorConfiguration) throws SensorInitializationException {
		...
		throw new SensorInitializationException("file retrieval type (" + FILE_RETRIEVAL_TYPE_KEY + ") not set or set to unknown value: " + type);
	}
	...
	private File checkForLocalDir(String source) throws SensorInitializationException {
		...
		throw new SensorInitializationException("file " + f + " either not found, or not an executable, or not a file.");
	}
	...
	private static int getRealPortNr(URI uri) {
		...
		if(schema == null) return port;
		if("http".equals(schema)) return 80;
		if("https".equals(schema)) return 443;
		return port;
	}
	...
	private File downloadFromUrl(String source) throws SensorInitializationException {
		...
		if(!uriAllowed(uri)){
			throw new SensorInitializationException("download uri not contained in ALLOWED set. " + httpAddresses);
		}
	...
	private void doDownloadFile(URI uri, File f) throws SensorInitializationException {
		try {
			...
			if(commandAvailable("wget")) {
				command = "wget";
				p = Runtime.getRuntime().exec("wget -o /dev/null -O " + f.getAbsolutePath() + " " + uri.toString());
			} else if(commandAvailable("curl")) {
				command = "curl";
				p = Runtime.getRuntime().exec("curl -o " + f.getAbsolutePath() + " " + uri.toString() + " >/dev/null");
			} else {
				throw new SensorInitializationException("cannot download file. neither wget nor curl was found.");
			}
			int i = p.waitFor();
			if(i != 0) {
				throw new SensorInitializationException("downloading with '" + command + "' failed. exit status: " + i);
			}
	...
	private File fileAsParameter(String source) throws SensorInitializationException {
		throw new SensorInitializationException("passing file as parameter currently not supported due to security concerns.");
	}
